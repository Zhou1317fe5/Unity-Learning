...
public class UDPServer : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (getNew == true)
        {
            ...
            //接收到的力传递给ARM
            //forceMagnitude = float.Parse(recvStr);
           
            if (float.TryParse(recvStr, out forceMagnitude))
            {
                Console.WriteLine(forceMagnitude);
            }
            else
            {
                Console.WriteLine("无法将字符串转换为浮点数");
            }
