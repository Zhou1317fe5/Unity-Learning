...
public class Q_UDP : MonoBehaviour
{
    ...
    [Header("存放对方的接收ip")]
    public string oppositeIP = "192.168.0.0";
    [Header("存放对方的接收端口")]
    ...
    void OnGUI()
    {
        ...
        string inputPort = "6000";
        inputPort = GUI.TextField(new Rect(155, 90, 80, 20), inputPort, 20);
        oppositePort = Convert.ToInt32(inputPort);

        GUI.Label(new Rect(65, 120, 80, 20), "发送的消息：");

        inputMessage = GUI.TextField(new Rect(155, 120, 100, 20), inputMessage, 20);

        GUI.Label(new Rect(65, 160, 80, 20), "接收到消息：");

        GUI.Label(new Rect(155, 160, 80, 20), recvStr);
        ...
        //if (GUI.Button(new Rect(65, 200, 60, 20), "开始连接"))
        //{
        //    SocketSendDefault(inputMessage);
        //}

        if (GUI.Button(new Rect(65, 240, 60, 20), "发送数据"))
        {
            SocketSendDefault(inputMessage);
        }
